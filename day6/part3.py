...
class State:
    ...
    def is_repeat(self, node):
        if node in self._visited:
            self._period = len(self._visited) - self._visited.index(node)
...
class Periods:
    def __init__(self):
        self._dict = dict()
    def add(self, period):
        k = period//4 # Must be a multiple of 4, i.e. 360 degrees
...
def test(p):
    ...
    while True:
        ...
        if end:
            break
        if lab.stopped():
            #print("[%3d] Guard in loop! Period %d" % (count, lab._history._period))
            loops.add(lab._history._period)
            success = True
            break
    lab.reset()
    return success
