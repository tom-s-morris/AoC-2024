...
class Node:
    ...
    def __iter__(self):
        # NOTE:
        # I started Using the itertools 'chain' to chain together the child iterators,
        # then switched to the depth-first approach below.
...
def count_trails():
    ...
    for th in trailheads:
        x = list(iter(th))
        summits = [n._visited for n in nodes if n._visited > 0 and n._height == 9]
        rating = sum(summits)
        print(th, rating)
        total_score += rating
        # reset visited state
